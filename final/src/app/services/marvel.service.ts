...
export class MarvelService {
  ...
  getPeliculas():Observable<any>{
    return this.http.get(`${URL_API}/series?ts=1&apikey=${KEY_PLUBLIC}&hash=${HASH}`).
    pipe(map((data:any)=>data.data.results));
  }

  getPeliculasId(id: string):Observable<any>{
    return this.http.get(`${URL_API}/series${id}?ts=1&apikey=${KEY_PLUBLIC}&hash=${HASH}`).
    pipe(map((data:any)=>data.data.results));
  }
